		...
		if data[focus_pos + offset].isPasswordProtected == true then
			MESSAGEMAN:Broadcast("OpenKeyboard", {
				...
				handler=function(params)
					...
					SYNCMAN:Send("joinLobby", {password = params.text, code=data[focus_pos + offset].code, machine=SYNCMAN:GetMachineState()})
				end
			})
		else
			
			SYNCMAN:Send("joinLobby", {code=data[focus_pos + offset].code, machine=SYNCMAN:GetMachineState()})
		end
	end
}
